...
export default function ChatWidget() {
    ...
    const [mood, setMood] = useState<"normal" | "roast" | "eli5">("normal");
                ...
                <AnimatePresence mode="wait">
                    {open ? (
                        <motion.svg key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.15 }}
                            xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-on-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className="fixed bottom-36 right-6 z-50 w-80 sm:w-96"
                    >
                        <div className="bg-gray-900 border border-gray-700/60 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                            style={{ height: "480px" }}>

                            {/* Header */}
                            <div className="flex items-center gap-3 px-4 py-3.5 bg-gray-900 border-b border-gray-700/50">
                                <div className="w-8 h-8 rounded-full bg-accent/20 border border-accent/40 flex items-center justify-center text-sm">
                                    🤖
                                </div>
                                ...
                                {messages.length > 1 && (
                                    <button
                                        onClick={() => setMessages([{ role: "assistant", content: "Chat cleared! Ask me anything about Uğurcan. 😊" }])}
                                ...
                                <button onClick={() => setOpen(false)} className="text-gray-500 hover:text-on-background transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
